...
pub fn instantiate(
    ...
    response = response.add_attribute("status", "start");
    // response = response.add_attribute("walk_name", msg.walk_name.clone());
    //

    let admin = msg.admin.clone().unwrap_or(info.sender.to_string()); // Use msg.admin or info.sender
    let admin_canonical = deps.api.addr_canonicalize(admin.as_str())?; // Canonicalize the admin address
    ADMIN.save(deps.storage, &admin_canonical)?; // Save the canonicalized admin address
    ...
    WALK_NAME.save(deps.storage, &msg.walk_name)?;
    MAX_TICKETS.save(deps.storage, &msg.max_tickets)?;
    TICKETS_SOLD.save(deps.storage, &0)?;
    CHECKPOINT_COORDS.save(deps.storage, &msg.checkpoint_coords)?;
    CHECKPOINT_NAMES.save(deps.storage, &msg.checkpoint_names)?;
    CHECKPOINT_HINTS.save(deps.storage, &msg.checkpoint_hints)?;
    BADGE_IMAGES.save(deps.storage, &msg.badge_images)?;
...
//     let sender_canonical = deps.api.addr_canonicalize(info.sender.as_str())?; // Convert info.sender to CanonicalAddr
//     if sender_canonical != admin {
//         return Err(StdError::generic_err(
//             "Unauthorized: Only admin can add a walk",
//         ));
//     }

//     // // Create public walk data
//     // let walk_public = WalkPublicData {
//     //     walk_name: walk_name.clone(),
//     //     max_tickets,
//     //     tickets_sold: 0, // Initially no tickets sold
//     // };

//     // // Create private walk data
//     // let walk_private = WalkPrivateData {
...
//     // // Store the walk data in public and private storage maps
//     // WALKS_PUBLIC.insert(deps.storage, &walk_name, &walk_public)?;
//     // WALKS_PRIVATE.insert(deps.storage, &walk_name, &walk_private)?;

//     Ok(Response::new()
//         .add_attribute("action", "add_walk")
//         .add_attribute("walk_name", walk_name))
// }
...
pub fn trigger_event(
    ...
    // check some_data to make sure it satisfies event
    // if some_data == "something" ...
    // else if not return an error

    // change the private metadata somehow...
    let mut private_attributes = priv_meta.extension.unwrap().attributes.unwrap();
    private_attributes.push(Trait {
        trait_type: Some("a new trait!".to_string()),
        value: "new trait value".to_string(),
        display_type: None,
        max_value: None,
    });
