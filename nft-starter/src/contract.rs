...
use crate::{expiration::Expiration, token::{Extension, Trait}};
...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> StdResult<Response> {
    ...
    let response = match msg {
        ...
        // **** Hackathon
        ExecuteMsg::TriggerEvent { token_id, some_data} => {
            trigger_event(
                deps,
                &env,
                &info.sender,
...
pub fn trigger_event(
    ...
    let mut pub_meta: Metadata = may_load(&pub_meta_store, &idx.to_le_bytes())?.unwrap_or(Metadata {
        token_uri: None,
        extension: None,
    });
    ...
    let mut priv_meta: Metadata = may_load(&priv_meta_store, &idx.to_le_bytes())?.unwrap_or(Metadata {
        token_uri: None,
        extension: None,
    });
    ...
    Ok(Response::new()
        .set_data(to_binary(&ExecuteAnswer::TriggerEvent { status: Success })?))
}
